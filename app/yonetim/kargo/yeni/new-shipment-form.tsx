...
export default function NewShipmentForm({
  ...
  useEffect(() => {
    if (state.shipmentId) {
      ...
      router.push(`/yonetim/kargo?created=${state.trackingNumber ?? state.shipmentId}`);
    }
    if (state.error) {
  ...
  return (
    <div>
      <PageHeader title="Yeni Kargo" description="Gönderi oluşturun" />
      <form
        action={onFormAction}
      ...
      >
        ...
        <FormSection title="Müşteri ve Kargo Firması">
          <div className="grid gap-4 sm:grid-cols-2">
            <Select label="Müşteri *" name="customerId" required>
              <option value="">Müşteri seçin</option>
              {customers.map((c) => (
                <option key={c.id} value={c.id}>
                  {c.name} ({c.type === "balance" ? "Bakiyeli" : "Cari"})
                </option>
              ))}
            </Select>
